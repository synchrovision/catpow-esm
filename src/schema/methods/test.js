...
import { getResolvedSchema } from "./getResolvedSchema.js";
...
export const test = (value, schema, rootSchema, params = {}) => {
	...
	switch (type) {
		...
		case "array": {
			...
			if (schema.prefixItems != null) {
				if (schema.prefixItems.some((itemSchema, index) => value[index] !== undefined && test(value[index], itemSchema, rootSchema, params) !== true)) {
	...
	if (onSuccess) {
		onSuccess(Object.assign({ schema, value }, params));
	}
	return true;
};
