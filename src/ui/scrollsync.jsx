...
export const scrollsync = function (el, param = {}) {
	...
	const updateActiveItem = () => {
		...
		if (app.param.controls && !app.param.loop) {
			if (app.param.controls.prev) {
				app.param.controls.prev.classList.toggle("is-disabled", index <= 0);
			}
			if (app.param.controls.next) {
				app.param.controls.next.classList.toggle("is-disabled", index >= l - 1);
			}
		}
		app.current = index;
	...
	const updateNavClass = () => {
		if (!app.param.nav || app.param.loop) {
			return;
		}
		const bnd = el.getBoundingClientRect();
	...
	if (app.param.controls) {
		if (app.param.controls.prev) {
			app.param.controls.prev.addEventListener("click", app.prev);
		}
		if (app.param.controls.next) {
			app.param.controls.next.addEventListener("click", app.next);
		}
	}
	if (app.param.loop) {
