...
export const Nav = (props) => {
	...
	if (undefined === Nav.State) {
		Nav.State = React.createContext({});
	}
	if (undefined === Nav.Functions) {
		Nav.Functions = React.createContext({});
	}
	...
	const deactivateItem = useCallback((item) => {
		if (item.active) {
			item.active = false;
			if (item.children) {
				item.children.forEach(deactivateItem);
			}
		}
	}, []);
	const initItem = useCallback((item) => {
		if (undefined === item.level) {
			item.level = 0;
		}
		if (item.children) {
			item.children.forEach((childItem, index) => {
				...
				itemSiblingsMap.set(
					childItem,
					item.children.filter((sibling) => sibling !== childItem)
				);
				initItem(childItem);
			});
		}
	...
	const init = useCallback((state) => {
		Object.values(menus).forEach((menu) => {
			menu.forEach((item, index) => {
				...
				itemSiblingsMap.set(
					item,
					menu.filter((sibling) => sibling !== item)
				);
				initItem(item);
			});
		});
	...
	const reducer = useCallback((state, action) => {
		switch (action.type) {
			case "activate": {
				...
				if (action.item.active && !action.updateLevel) {
					return state;
				}
			...
			case "deactivate": {
				...
				if (!itemParentMap.has(action.item)) {
					return { ...state, activeItem: false, activeMenu: false, currentLevel: 0 };
				}
	...
	return (
		<Nav.Functions.Provider value={functions}>
			<Nav.State.Provider value={{ menus, state, dispatch }}>{props.children}</Nav.State.Provider>
