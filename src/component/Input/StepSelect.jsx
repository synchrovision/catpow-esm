...
import { Bem, Popup } from "catpow/component";
					...
					<ul className="_items">
						{state.selectedItems && state.selectedItems.length ? (
							state.selectedItems.map((item) => (
								<li className="_item" onClick={() => toggleItem(item)} key={item.value}>
									{item.label}
									<span className="_button"></span>
								</li>
							))
						) : (
						...
						<div className="_body">
							<div className="_selections">
								{state.activeItems.map((activeItem, index) => (
									<div className="_options" key={index}>
										<ul className="_items">
											{activeItem.options.map((item, index) => (
												<li
													className={clsx("_item", {
										...
										<div className={clsx("_arrow", index < state.activeItems.length - 1 ? "is-shown" : "is-hidden")}></div>
									</div>
								))}
							</div>
						</div>
