...
export const Honycomb = (props) => {
	...
	const Hexagon = useCallback(
		(props) => {
			const { className, ...dynamicProps } = cellCallback(props);
			return <path className={classes._cell(className)} d={`M ${props.x} ${props.y} ${drawHexagon}`} {...otherProps} {...dynamicProps} />;
		},
		[cellCallback, otherProps, classes]
	);

	const cells = useMemo(() => {
		const stepX = Math.sqrt(3) * (size + gap / 2),
			stepY = (size + gap / 2) * 1.5,
			cells = [];
		let row,
