export const observeIntersection = function (items, param = {}) {
	...
	app.observer = new IntersectionObserver((entries) => {
		entries.forEach((entry) => {
			...
			if (r > prev) {
				if (prev === 0) {
					el.dispatchEvent(new Event("enterIntoView"));
				}
				if (prev < 0.5) {
					if (r > 0.5) {
						el.dispatchEvent(new Event("enterHalfIntoView"));
					}
				}
				if (r === 1) {
					el.dispatchEvent(new Event("enterFullIntoView"));
				}
			} else {
				if (r === 0) {
					el.dispatchEvent(new Event("leaveFullFromView"));
				} else if (prev > 0.5) {
					if (prev === 1) {
						el.dispatchEvent(new Event("leaveFromView"));
					}
					if (r < 0.5) {
						el.dispatchEvent(new Event("leaveHalfFromView"));
					}
				}
			}
		});
