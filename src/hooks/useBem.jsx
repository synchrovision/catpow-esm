...
const revertBlockClassNameRecursive = (el) => {
	for (const child of el.children) {
		if (!elementClassMap.has(child) || (blockClassMap.has(child) && keyClassMap.get(child).slice(-1) === "-")) {
			continue;
		}
...
const revertElementClassNameRecursive = (el) => {
	for (const child of el.children) {
		if (!elementClassMap.has(child) || blockClassMap.has(child) || keyClassMap.get(child).slice(-1) === "_") {
			continue;
		}
		child.classList.replace(elementClassMap.get(child), keyClassMap.get(child));
